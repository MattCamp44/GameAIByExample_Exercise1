                                        ...
                                        void*  ExtraInfo)
{
    ...
    if (delay <= 0.0f)
    {
        cout << "\nInstant telegram dispatched at time: " << Clock->GetCurrentTimeCrudeTimer()
    ...
    {
        double CurrentTime = Clock->GetCurrentTimeCrudeTimer();
        ...
        cout << "\nDelayed telegram from " << GetNameOfEntity(pSender->ID()) << " recorded at time "
             << Clock->GetCurrentTimeCrudeTimer() << " for " << GetNameOfEntity(pReceiver->ID())
             << ". Msg is "<< MsgToStr(msg);
